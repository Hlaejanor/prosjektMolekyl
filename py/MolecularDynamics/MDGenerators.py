...
def PopulateCreateCrystalStructure(n, periodicCode,  distance, N, mass, temperature = None):
    print(f"Computes {n}x{n}x{n} tetrahedral crystal lattice with cell distance {distance}")
        ...
        for j in range(n):
            for k in range(n):
                ...
                for lstr in faceCenteredCubicLattice:
                    particles.append(MDParticle.Particle('ijk', periodicCode, distance * lstr + cellPos, v0[index], N, mass))
                    index +=1
